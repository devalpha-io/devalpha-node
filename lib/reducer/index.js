...
export default (state = initialState, action) => {
  ...
  case ORDER_FILLED: {
    ...
    nextState = nextState.set('total', buildTotal(nextState))
    nextState = nextState.set('returnsTotal', buildReturnsTotal(nextState))
    nextState = nextState.set('returnsPeriod', buildReturnsPeriod(nextState))
    nextState = nextState.set('returnsPeriod', buildReturnsPeriod(nextState))
    nextState = nextState.setIn(['metrics', 'maxDrawdown'], buildMaxDrawdown(nextState))
    // nextState = nextState.setIn(['metrics', 'sharpeRatio'], buildSharpeRatio(nextState))
