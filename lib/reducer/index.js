...
export default (state = initialState, action) => {
  ...
  case ORDER_PLACED: {
    ...
    if (sign(order.quantity) === 1) {
      ...
      nextState = nextState.set('reservedCash', reservedCash)
      nextState = nextState.set('cash', cash)
    }
  ...
  case ORDER_FILLED: {
    ...
    nextState = nextState.merge({
      total: buildTotal(nextState),
      returnsTotal: buildReturnsTotal(nextState),
      returnsPeriod: buildReturnsPeriod(nextState)
    })
  ...
  case ORDER_CANCELLED: {
    ...
    if (sign(cancelledOrder.quantity) === 1) {
      ...
      /* reservedCash = reservedCash - cost - commission */
      const reservedCash = n(chain(b(nextState.get('reservedCash')))
        .subtract(b(cost)).subtract(b(cancelledOrder.commission)).done())

      /* cash = cash + cost + commission */
      const cash = n(chain(b(nextState.get('cash')))
    ...
    nextState = nextState.deleteIn(['orders', cancelledOrder.id.toString()])
    return nextState
  }
  ...
  case BAR_RECEIVED: {
    ...
    if (!nextState.hasIn(['positions', identifier])) {
      ...
      return nextState.set('history', buildHistory(nextState))
    }
    ...
    nextState = nextState.set('history', buildHistory(nextState))
